...
logger.info(`Working hours check: withinHours=${withinHours}, settings=${JSON.stringify(settings?.working_hours)}`)
        ...
        if (customer?.id) {
          ...
          if (activeBookingRes.rows.length > 0) {
            return { success: false, message: `You already have an active booking with Dr. ${doctor.name} today.` }
          }
        }
